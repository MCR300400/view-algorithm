<template>
    <div class="bfs-component">
      <h2>Configurazione BFS e Visualizzazione</h2>
      ...
      <button @click="startBfs">Avvia BFS</button>
  ...
  export default {
    ...
    methods: {
      ...
      startBfs() {
        ...
        // Avvia l'algoritmo BFS e salva tutti i passaggi intermedi
        this.bfs();
        ...
        }, 1); // Aggiorna ogni 0.5 secondi
      },
      bfs() {
        const queue = [[...this.array]]; // Inizia con l'array originale
        const seen = new Set();
        seen.add(this.array.join(','));
  
        // Limita il numero di passaggi salvati per evitare overflow
        const maxSteps = 100000; // Limita a 1000 passaggi
  
        while (queue.length && this.sortingSteps.length < maxSteps) {
          const currentArray = queue.shift();
          this.sortingSteps.push([...currentArray]);
  
          // Simula l'ordinamento BFS: scambia coppie di elementi
          for (let i = 0; i < currentArray.length - 1; i++) {
            const subArray = [...currentArray];
            [subArray[i], subArray[i + 1]] = [subArray[i + 1], subArray[i]]; // Scambia coppie
